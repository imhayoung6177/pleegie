...
// ✅ WithdrawModal 바로 위에 이 함수 추가 , 위도 경도 반환 함수
const convertCoordsToAddress = async (latitude, longitude) => {
  try {
    // x = 경도(longitude), y = 위도(latitude) ← 순서 중요!
  ...
  <div className="mp-modal-overlay" onClick={onClose}>
    <div className="mp-modal" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="mp-modal-btns">
        <button className="mp-modal-cancel" onClick={onClose}>
          취소
        </button>
        <button className="mp-modal-confirm" onClick={onConfirm}>
          탈퇴하기
        </button>
      </div>
    </div>
...
if (response.ok) {
  ...
  if (loading && activeTab === "main")
    return <div className="mypage-loading">정보 로딩 중...</div>;
      ...
      <div className="mp-profile-card">
        ...
        <div className="mp-profile-info">
          ...
          <div className="mp-role-row">
            <div className="mp-profile-role">
              {userInfo.role === "MARKET" ? "소상공인" : "일반 회원"}
            </div>
            <button
              className="mp-withdraw-btn"
